...
resource "aws_security_group" "data" {
  ...
  # MISCONFIG #3 (networking), continued: DB port also opened to the entire
  # internet. publicly_accessible = true on the RDS instance (database.tf)
  # alone was not enough to make Prowler's "RDS instance is not publicly
  # exposed to the Internet" check fail -- that check evaluates security-group
  # reachability, not just the instance flag. This 0.0.0.0/0 ingress rule is
  # what actually makes the exposure real and detectable. To revert: delete
  # this ingress block (the web-tier-only rule above is sufficient on its own).
  ingress {
    description = "MISCONFIG #3: DB port opened to the entire internet"
    from_port   = var.db_engine == "postgres" ? 5432 : 3306
    to_port     = var.db_engine == "postgres" ? 5432 : 3306
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
